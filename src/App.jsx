...
function buildGrid() {
  ...
  for (let x = 8; x <= 16; x++) { g[9][x] = FLOOR; g[15][x] = FLOOR; }
  for (let y = 9; y <= 15; y++) { g[y][8] = FLOOR; g[y][16] = FLOOR; }
  for (let y = 10; y <= 14; y++) for (let x = 9; x <= 15; x++) g[y][x] = COMMON;
...
export default function ForbesJags() {
    ...
    setGame((g) => {
      ...
      if (roll < (boosted ? 0.65 : 0.42)) {
        ...
        if (!hasAli && next.totalCollected >= ALI_TRIGGER_ITEMS && next.extraRAs.length < MAX_EXTRA_RAS) {
          next.extraRAs = [...g.extraRAs, makeRA(g.nextRAId, RA_ROSTER[1], g.raSprites.Ali)];
          next.nextRAId = g.nextRAId + 1;
          next.message += " Ali is now on duty!";
        }
  ...
  return (
    <div style={styles.page}>
      ...
      {game.phase === "intro" && <IntroScreen onStart={goToSetup} />}
    ...
    <div style={styles.introWrap}>
      <div style={styles.introCard}>
        ...
        <p style={styles.introText}>
          ...
          best gathering spot on campus.
        </p>
        <p style={styles.introText}>
          Kevin, your RA, patrols the halls and really wants everyone at his RA
          ...
          the very first time you're caught, no-nonsense Vanessa joins too. Get
          caught carrying supplies three times and you're busted — the party's
          over. Say yes to an RA Program and everyone freezes in place for a
          few seconds — safe passage. Use the hallway tunnel and the loop
          around the common room to shake anyone on your tail.
        </p>
          ...
          <div style={styles.spriteRow}>
            {PLAYER_SPRITES.map((sp) => (
              <button
                key={sp}
                className="fj-btn"
                onClick={() => setSetup((s) => ({ ...s, playerSprite: sp }))}
                style={{ ...styles.spriteBtn, background: setup.playerSprite === sp ? "#e2a13b" : "#2f2657" }}
              >
                {sp}
              </button>
            ))}
  ...
  return (
    <div style={styles.introWrap}>
      <div style={styles.introCard}>
        ...
        <div style={styles.subtitle}>{won ? "Forbes 3 will talk about this one for weeks." : loseText}</div>
        <p style={styles.introText}>
  ...
  return (
    <div style={styles.hud}>
      <div style={styles.hudRow}>
        <div style={styles.hudStat}>{game.playerSprite} {game.playerName}</div>
      ...
      <div style={styles.hudRow}>
        <span style={styles.alertLabel}>KEVIN ALERT</span>
        ...
        <div style={{ ...styles.board, aspectRatio: `${COLS} / ${ROWS}` }}>
          ...
          {ROOMS.map((room) => {
            ...
            const icon =
              ...
              state === "studying" ? "📚" : "";
            return (
              <div key={room.id} title={`Room ${ROOM_NUMBER(room.id)}`} style={{
                position: "absolute",
          ...
          <div style={{
            ...
            <div style={{
              ...
              display: "flex", alignItems: "center", justifyContent: "center",
              fontSize: "0.55em", boxShadow: "0 0 0 2px #1c1533",
