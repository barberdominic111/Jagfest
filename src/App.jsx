    ...
    <div style={styles.page}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        @keyframes fj-ringpulse { 0%,100% { opacity: 0.6; } 50% { opacity: 1; } }
...
function clockColor(fraction) {
  if (fraction > 0.5) return "#6ea36a";
  if (fraction > 0.2) return "#e2a13b";
  return "#d1453b";
}

function threatRing(alert) {
  if (alert < 33) return { stroke: "#6ea36a", dash: "2,7", opacity: 0.7, pulse: false };
  if (alert < 66) return { stroke: "#e2a13b", dash: "3,4.5", opacity: 0.85, pulse: false };
  return { stroke: "#d1453b", dash: "5,2", opacity: 1, pulse: alert >= 80 };
}

function ClockTimer({ timeLeft }) {
  const fraction = Math.max(0, Math.min(1, timeLeft / GAME_SECONDS));
  const color = clockColor(fraction);
  const dashoffset = CLOCK_C * (1 - fraction);
  return (
    <div style={styles.clockWrap}>
      <svg width="54" height="54" viewBox="0 0 54 54" style={{ transform: "rotate(-90deg)" }}>
        <circle cx="27" cy="27" r={CLOCK_R} fill="none" stroke="#3a3060" strokeWidth="6" />
        <circle cx="27" cy="27" r={CLOCK_R} fill="none" stroke={color} strokeWidth="6"
          strokeDasharray={CLOCK_C} strokeDashoffset={dashoffset} strokeLinecap="round"
          style={{ transition: "stroke-dashoffset 1s linear" }} />
      </svg>
      <div style={{ ...styles.clockTime, color }}>{timeLeft}s</div>
    </div>
  );
}

function RaRosterMini({ game }) {
  const onDutyNames = new Set([game.kevin.name, ...game.extraRAs.map((r) => r.name)]);
  return (
    <div style={styles.raMiniCol}>
      {RA_ROSTER.map((r) => {
        const on = onDutyNames.has(r.name);
        const icon = game.raSprites[r.name];
        return (
          <div key={r.name} style={styles.raMiniRow} title={`${r.name} — ${on ? "on duty" : "not on duty yet"}`}>
            <div style={{ ...styles.raMiniIcon, background: on ? r.color : "#2f2657", opacity: on ? 1 : 0.45, filter: on ? "none" : "grayscale(1)", boxShadow: on ? `0 0 6px 2px ${r.color}99` : "none" }}>
              {icon}
            </div>
          </div>
        );
...
function StrikesMini({ strikes }) {
  return (
    <div style={styles.pips}>
      {Array.from({ length: MAX_STRIKES }).map((_, i) => (
        <div key={i} style={{ ...styles.pip, background: i < strikes ? "#d1453b" : "#3a3060" }}>✕</div>
      ))}
    </div>
  );
}

function ScoreMini({ delivered, winTarget }) {
  const pct = Math.min(100, Math.round((delivered / winTarget) * 100));
  return (
    <div style={styles.scoreMiniWrap}>
      <div style={styles.scoreMiniText}>{delivered}/{winTarget}</div>
...
function PlayScreen({ game, pressDir, releaseDir, activeBtn, knock, onMenu }) {
  ...
  const ring = threatRing(game.alert);
  ...
  return (
    <div style={styles.playWrap}>
      ...
      <div style={styles.stage}>
        <div style={{ ...styles.corner, gridArea: "tl" }}>
          <div style={styles.cornerLabel}>TIME</div>
          <ClockTimer timeLeft={game.timeLeft} />
        </div>
        <div style={{ ...styles.corner, gridArea: "tr" }}>
          <div style={styles.cornerLabel}>RAs</div>
          <RaRosterMini game={game} />
        </div>
        <div style={{ ...styles.corner, gridArea: "bl" }}>
          <div style={styles.cornerLabel}>STRIKES</div>
          <StrikesMini strikes={game.strikes} />
        </div>
        <div style={{ ...styles.corner, gridArea: "br" }}>
          <div style={styles.cornerLabel}>PARTY</div>
          <ScoreMini delivered={game.delivered} winTarget={game.winTarget} />
        </div>

        <div style={{ gridArea: "board", position: "relative" }}>
          <svg style={styles.threatRingSvg} viewBox="0 0 100 100" preserveAspectRatio="none">
            <rect x="1.5" y="1.5" width="97" height="97" rx="3" fill="none"
              stroke={ring.stroke} strokeWidth="1.8" strokeDasharray={ring.dash} opacity={ring.opacity}
              className={ring.pulse ? "fj-ring-pulse" : undefined} />
          </svg>
      ...
      <div style={styles.carriedStrip}>
        {game.carried.length === 0
          ? <span style={styles.hudMuted}>Carrying nothing</span>
          : game.carried.map((it, i) => <span key={i} style={styles.carriedIcon}>{ICON[it]}</span>)}
      </div>
      <div style={styles.messageStrip}>{game.message || objective}</div>
...
const styles = {
  ...
  playWrap: { width: "100%", maxWidth: 700, display: "flex", flexDirection: "column", alignItems: "center", gap: 6 },
  ...
  menuBtn: { fontFamily: "'Press Start 2P', monospace", fontSize: "0.38em", color: "#ead9b3", background: "#2f2657", borderRadius: 4, padding: "5px 8px" },
  carriedStrip: { width: "100%", display: "flex", gap: 5, fontSize: "1.05em", minHeight: 20, background: "#241b3a", border: "2px solid #2f2657", borderRadius: 6, padding: "3px 8px", alignItems: "center" },
  carriedIcon: { filter: "drop-shadow(0 0 2px #000)" },
  hudMuted: { color: "#8fa3d1", fontSize: "0.75em" },
  messageStrip: { width: "100%", fontSize: "0.9em", color: "#e2a13b", background: "#1c1533", padding: "4px 8px", borderRadius: 4, border: "1px solid #2f2657", textAlign: "center", minHeight: "1.2em" },
  ...
  stage: {
    ...
    gridTemplateAreas: `"tl board tr" "bl board br"`,
    gap: 5,
    alignItems: "stretch",
  },
  corner: {
    background: "#241b3a",
    border: "2px solid #2f2657",
    borderRadius: 6,
    padding: "4px 2px",
    display: "flex",
  ...
  cornerLabel: { fontFamily: "'Press Start 2P', monospace", fontSize: "0.32em", color: "#8fa3d1" },
  clockWrap: { position: "relative", width: 40, height: 40 },
  clockTime: { position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Press Start 2P', monospace", fontSize: "0.32em" },
  raMiniCol: { display: "flex", flexDirection: "column", gap: 4 },
  raMiniRow: { display: "flex", justifyContent: "center" },
  raMiniIcon: { width: 22, height: 22, borderRadius: 4, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.6em" },
  pips: { display: "flex", flexDirection: "column", gap: 3, alignItems: "center" },
  pip: { width: 16, height: 16, borderRadius: 3, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.5em", color: "#fff" },
  scoreMiniWrap: { display: "flex", flexDirection: "column", alignItems: "center", gap: 3, width: "100%" },
  scoreMiniText: { fontFamily: "'Press Start 2P', monospace", fontSize: "0.38em", color: "#e2a13b" },
  scoreBar: { width: "85%", height: 6, background: "#3a3060", borderRadius: 3, overflow: "hidden" },
  scoreBarFill: { height: "100%", background: "#6ea36a" },
  threatRingSvg: { position: "absolute", inset: -7, width: "calc(100% + 14px)", height: "calc(100% + 14px)", pointerEvents: "none" },
  board: { position: "relative", width: "100%", border: "4px solid #2f2657", borderRadius: 4, overflow: "hidden" },
